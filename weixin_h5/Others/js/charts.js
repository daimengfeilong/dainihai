...
function reset() {
	var itemval = "";
	var localcc = localStorage.getItem(t);
	//console.log(localcc);
	if (localcc.indexOf("|&zx&|") > -1) {
		var localcc_2 = localcc.split('|&zx&|');
		for (var i = 0; i < localcc_2.length; i++) {
			var localcc_3 = localcc_2[i].split(',');
			itemval += "{ label: '" + localcc_3[0] + "', value: " + localcc_3[1] + " },";
		}
	} else {
		var localcc_1 = localcc.split(',');
		itemval = "{ label: '" + localcc_1[0] + "', value: " + localcc_1[1] + " },";
	...
	console.log(itemval);
	var dataObj = eval("[" + itemval + "]"); //转换为json对象 
	//console.log(dataObj);
	chart.populate(dataObj);
	setTimeout(loadxian, 120000);
}